...
import firstSpring.practice.practice.config.auth.dto.OAuthAttributes;
import firstSpring.practice.practice.config.auth.dto.SessionUser;
import firstSpring.practice.practice.domain.user.User;
import firstSpring.practice.practice.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        //OAuthAttributes : OAuth2User attribute 담을 클래스
        User user = saveOrUpdate(attributes);
        httpSession.setAttribute("user", new SessionUser(user));
        // SessionUser : 세선에 사용자 정보를 저장하기 위한 DTO 클래스
